...
final class OwnersForm extends SimpleForm implements UIForm {
    ...
    OwnersForm(Region region, Player player) {
        ...
        if (player.hasPermission("sregionprotector.admin") || this.region.isCreator(player.getName())) {
            region.getOwners().forEach(owner -> this.addElement(new Button(owner, OwnerRemoveForm.class, owner, region, player)));
        } else {
            region.getOwners().forEach(owner -> this.addElement(new Button(owner, OwnersForm.class, region, player)));
        }
