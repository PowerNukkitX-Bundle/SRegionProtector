...
final class OwnerRemoveForm extends SimpleForm implements UIForm {
    ...
    OwnerRemoveForm(String owner, Region region, Player player) {
        ...
        this.region = region;
        this.addElement(new Button("Yes", OwnersForm.class, region, player).setBeforeNext(() -> {
            if (region.isOwner(owner)) regionManager.removeOwner(region, owner);
        }));
        this.addElement(new Button("No", OwnersForm.class, region, player));
    }
